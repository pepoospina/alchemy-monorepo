...
const PeriodRow = (props: IProps) => {
  ...
  const [repuationRewardForLockings, setRepuationRewardForLockings] =
    React.useState(new Decimal(0));
  ...
  React.useEffect(() => {
    ...
    if (period < currentLockingBatch) {
      setRedeemableAmount(
        redeemableAmount + repuationRewardForLockings.toNumber()
      );
    }
  ...
  for (const lock of lockData) {
    const lockingBatch = getLockingBatch(
      Number(lock.lockingTime),
      Number(schemeParams.startTime),
      schemeParams.batchTime
    );
    if (
      lockingBatch <= period &&
      lockingBatch + Number(lock.period) >= period
    ) {
  ...
  React.useEffect(() => {
    const getRepuationData = async () => {
      const repuationRewardForBatch =
        await props.cl4rScheme.getRepuationRewardForBatch(
          schemeParams.repRewardConstA,
          schemeParams.repRewardConstB,
          period
        );
      setRepuationRewardForBatch(
        repuationRewardForBatch.div(new Decimal(WEI) as any) as any
      );
      const repuationRewardForLockings =
        await props.cl4rScheme.getReputationRewardForLockingIds(
          lockingIds,
          period,
          repuationRewardForBatch
        );
      setRepuationRewardForLockings(
        repuationRewardForLockings.div(new Decimal(WEI) as any) as any
      );
    };
    getRepuationData();
  }, [period, lockData]);
  ...
  const reputationToReceive = isPeriodRedeemed ? (
    <div className={css.redeemedLabel}>Redeemed</div>
  ) : (
    formatTokens(toWei(repuationRewardForLockings.toNumber()), "REP")
  );
  ...
  return (
    <tr className={css.row}>
      ...
      <td>
        {inProgress ? (
          <span className={css.inProgressLabel}>In Progress</span>
        ) : (
          reputationToReceive
        )}
      </td>
